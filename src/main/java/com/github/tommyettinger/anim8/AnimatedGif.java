        ...
        try {
            ...
            for (int i = 0; i < analyzedPixmapArray.size; i++) {
                if (firstFrame) {
                    ...
                    writePalette(analyzedPixmapArray.get(0).getColorTab()); // global color table
                    if (repeat >= 0) {
                ...
                if (!firstFrame) {
                    writePalette(analyzedPixmapArray.get(i).getColorTab()); // local color table
                }
                writePixels(analyzedPixmapArray.get(i).getIndexedPixels()); // encode and write pixel data
                firstFrame = false;
            }
        } catch (IOException e) {
            ok = false;

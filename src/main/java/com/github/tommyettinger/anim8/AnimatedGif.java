...
public class AnimatedGif implements AnimationWriter, Dithered {
    ...
    /**
     * If true (the default), {@link #addFrames(Array)} will use a {@link ForkJoinPool} to analyze frames in parallel.
     * If false, frames will be analyzed sequentially.
     */
    public boolean useParallel = true;
     ...
     * @param ims an Array of Pixmap frames that should all be the same size, to be written in order
    ...
    public boolean addFrames(Array<Pixmap> ims) {
        ...
        try {
            ...
            images = ims;
            colorDepth = 8;
            AnalyzeTaskRecursiveOptimized analyzeTask = new AnalyzeTaskRecursiveOptimized(0, images.size, images, palette, ditherAlgorithm, ditherStrength, fastAnalysis, flipY);
            ...
            for (int i = 0; i < arr.size; i++) {
                AnalyzedPixmap analyzed = arr.get(i);
                if (firstFrame) {
                    writeLSD(); // logical screen descriptor
                    writePalette(analyzed.getColorTab()); // global color table
                    if (repeat >= 0) {
                        // use NS app extension to indicate reps
                        writeNetscapeExt();
                    }
                }
                updateTransIndex(analyzed);
                writeGraphicCtrlExt(); // write graphic control extension
                writeImageDesc(); // image descriptor
                if (!firstFrame) {
                    writePalette(analyzed.getColorTab()); // local color table
                }
                writePixels(analyzed.getIndexedPixels()); // encode and write pixel data
                firstFrame = false;
            }
        } catch (IOException e) {
            ok = false;
    ...
    /**
     * Adds a single frame to the GIF. Invoking <code>finish()</code> flushes all frames. If
    ...
    public boolean addFrame(Pixmap im) {
        ...
        try {
            ...
            if (firstFrame) {
                writeLSD(); // logical screen descriptor
                writePalette(colorTab); // global color table
                if (repeat >= 0) {
                    // use NS app extension to indicate reps
                    writeNetscapeExt();
                }
            ...
            writeImageDesc(); // image descriptor
            if (!firstFrame) {
                writePalette(colorTab); // local color table
            }
    ...
    protected void analyzePixels() {
        AnalyzePixels task = new AnalyzePixels(seq - 1, image, palette, ditherAlgorithm, ditherStrength, fastAnalysis, flipY);
        AnalyzedPixmap analyzed = task.analyzePixels();
        this.indexedPixels = analyzed.getIndexedPixels();
        this.colorTab = analyzed.getColorTab();
        updateTransIndex(analyzed);
        colorDepth = 8;
        palSize = 7;
    }
    
    protected void updateTransIndex(AnalyzedPixmap analyzed) {
        // Check if the first entry in color table is transparent (0,0,0)
        // Note: this is a heuristic. AnalyzePixels should ideally report if it used the transparent index.
        if (analyzed.getColorTab() != null && analyzed.getColorTab().length >= 3) {
            if (analyzed.getColorTab()[0] == 0 && analyzed.getColorTab()[1] == 0 && analyzed.getColorTab()[2] == 0) {
                transIndex = 0;
            } else {
                transIndex = -1;
            }

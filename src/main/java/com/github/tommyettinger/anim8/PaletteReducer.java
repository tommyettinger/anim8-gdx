...
//        }
        ...
        try {
            ...
            for (int i = 0; i < 4096; i++) {
                TRI_BLUE_NOISE_MULTIPLIERS[i] = Math.exp(OtherMath.probit((PaletteReducer.TRI_BLUE_NOISE[i] + 128.5) * 0x1p-8) * 0.78125);
            }
...
//                            adj = (pos * pos - 0.3f) * strength;
    ...
    public Pixmap reduceScatter (Pixmap pixmap) {
        ...
        int er, eg, eb;
        byte paletteIndex;
        float w1 = (float)(ditherStrength * populationBias * 0.1), w3 = w1 * 3f, w5 = w1 * 5f, w7 = w1 * 7f;
        for (int y = 0; y < h; y++) {
            int ny = y + 1;
            for (int i = 0; i < lineLen; i++) {
                curErrorRed[i] = nextErrorRed[i];
